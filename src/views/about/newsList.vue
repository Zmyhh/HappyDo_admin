<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.category" style="width: 200px" class="filter-item" placeholder="请选择类目" clearable>
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="首页是否显示" prop="isOnDisplay">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.category != 'Banner图片'" :type="scope.row.isOnDisplay ? 'success' : 'error' ">{{ scope.row.isOnDisplay ? '显示' : '隐藏' }}</el-tag>
        </template>
      </el-table-column>
    ...
    <!-- 添加或修改对话框 -->
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="detailDialogVisible" width="900px">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <div v-else>
          ...
          <el-form-item label="图片" prop="picUrl">
            ...
            <span v-if="dataForm.category == 'Banner图片'" class="avatar-tip">最佳尺寸：1920*761；大小推荐不超过300kb</span>
            <span v-else class="avatar-tip">最佳尺寸：50*50；大小推荐不超过50kb</span>
          </el-form-item>
          <el-form-item label="标题" prop="title">
            <el-input v-model="dataForm.title"/>
          </el-form-item>
          <el-form-item label="类目" prop="category">
            <el-select v-model="dataForm.category" disabled style="width: 300px" class="filter-item" placeholder="类目">
              <el-option v-for="(key, value) in categoryMap" :key="key" :label="key" :value="value"/>
            </el-select>
          </el-form-item>
          ...
          <el-form-item v-if="dataForm.category != 'Banner图片'" label="首页是否显示" prop="isOnDisplay">
            <el-select v-model="dataForm.isOnDisplay" style="width: 300px" class="filter-item" placeholder="首页是否显示">
              <el-option v-for="(key, value) in isOnDisplayMap" :key="key" :label="key" :value="value"/>
            </el-select>
          </el-form-item>
          <el-form-item v-if="dataForm.category != 'Banner图片'" label="新闻简介" style="width: 800px">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-else type="primary" @click="continueData">确定</el-button>
      </div>
    </el-dialog>
...
<script>
import { list, Delete, create, update } from '@/api/about'
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.resetForm()
      this.dialogStatus = 'createContinue'
    ...
    handleUpdate(row) {
      ...
      this.detailDialogVisible = true
      if (typeof this.dataForm.category === 'undefined') {
        this.detailVisible = true
        this.dialogStatus = 'updateContinue'
      }
      this.$nextTick(() => {
        this.$refs['dataForm'].clearValidate()
      })
    ...
    continueData() {
      ...
      if (this.dialogStatus === 'updateContinue') {
        this.dialogStatus = 'update'
      }
      if (this.dialogStatus === 'createContinue') {
        this.dialogStatus = 'create'
      }
      if (this.dataForm.category === 'Banner图片') {
        this.dataForm.isOnDisplay = 'true'
      }
    },
    handleDelete(row) {
