  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="语言" prop="language">
        <template slot-scope="scope">
          <el-tag :type="scope.row.language ? 'chinese' : 'english' ">{{ scope.row.language ? '中文' : '英文' }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="detailDialogVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="图片" prop="picUrl">
          ...
          <span class="avatar-tip">最佳尺寸：1920*761；大小推荐不超过300kb</span>
        </el-form-item>
        <el-form-item label="标题" prop="title">
          <el-input v-model="dataForm.title"/>
        </el-form-item>
        <el-form-item label="语言" prop="language">
          <el-select v-model="dataForm.language" style="width: 300px" class="filter-item" placeholder="语言">
            <el-option v-for="(key, value) in languageMap" :key="key" :label="key" :value="value"/>
          </el-select>
        </el-form-item>
...
import Global_language from '@/components/Language/language'
...
export default {
  name: 'BannerList',
  components: { BackToTop, Pagination, Global_language },
  filters: {
  ...
  data() {
    return {
      ...
      ids: [],
      languageMap: Global_language.languageMap
    }
  },
  computed: {
    headers() {
